...
class Model{
	...
	public static function getAllOffre($id){
		...
		$req_prep->execute();
		$req_prep->setFetchMode(PDO::FETCH_CLASS, 'Model'.ucfirst('offreemploi'));
	...
	public static function getCandidature($id){
		...
		$req_prep = Model::$pdo->prepare($sql);
		$req_prep->execute();
		$req_prep->setFetchMode(PDO::FETCH_CLASS, 'Model'.ucfirst('candidature'));
			  if ($req_prep->rowCount() != 0){
				$rslt = $req_prep->fetchAll();
				return $rslt;
				  die();
  ...
  public static function login($username){
	...
	$req_prep->execute();
	$req_prep->setFetchMode(PDO::FETCH_CLASS, 'Model'.ucfirst('users'));
		  if ($req_prep->rowCount() != 0){
			$rslt = $req_prep->fetch();
			return $rslt;
			  die();
